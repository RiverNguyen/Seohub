...
import SectionField from '@/app/(main)/_components/process-and-field/SectionField'
      ...
      <div id='warpper_section'>
        ...
        >
              ...
              {workflow.tag && (
                <div className='badge__label flex flex-col m-[0.625rem] w-max absolute top-[-25%]'>
                  <h2
                    ...
                    className='badge__label-text relative flex justify-center items-center bg-[#1550e5] whitespace-nowrap w-0 h-[3.04881rem] px-[0.48138rem] text-white text-[1.625rem] font-normal leading-[134%] overflow-hidden transition-all duration-[0.75s] ease-in-out opacity-0'
                  ></h2>
                </div>
              )}
              <div
          ...
          <div className='overflow-x-hidden absolute bottom-0 flex w-full border-t border-[#e6e8ea] z-10'>
            {workflow?.workflow_list?.map((item, index: number) => {
              const numberText = String(index + 1).padStart(2, '0')

              return (
                <div
                  key={index}
                  className='item-snapping flex-shrink-0 flex flex-col justify-between overflow-hidden p-5 pl-[1.875rem] w-[34.25rem] h-[22.6875rem] border-l border-[#e6e8ea] bg-gradient-to-b from-[#548beb] to-[#0059f1]'
                >
            ...
            >
              ...
              <div className='last-item__btn flex flex-col w-[15.6rem]'>
                <svg
                  xmlns='http://www.w3.org/2000/svg'
                  width='110'
                  height='111'
